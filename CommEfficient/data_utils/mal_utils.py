...
def fetch_mal_data(test_images, test_targets, args, num_classes, images_per_client):
    ...
    #print("Allowed source labels", allowed_source_labels)
    ...
    return mal_data, mal_labels, y

def fetch_test_data(test_images, test_targets, allowed_source_labels, num_mal_images):
    true_images = []
    true_labels = []
    if len(np.unique(allowed_source_labels)) == 1:
        idxs = np.where(test_targets == allowed_source_labels[0])[0]
        true_images = test_images[idxs][:num_mal_images]
        true_labels = test_targets[idxs][:num_mal_images]
    else:
...
def fetch_source_idxs(test_images, args, num_classes, num_mal_images):
    if args.mal_type in ["A", "B"]:
        ...
        return np.array([7 for _ in range(num_mal_images * 2)])
    else:
        assert False
